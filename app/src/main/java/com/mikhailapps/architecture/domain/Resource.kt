package com.mikhailapps.architecture.domain

sealed class Resource<T> {

    data class Idle<T>(val data: T? = null) : Resource<T>()

    data class Progress<T>(val progression: Int?, val data: T?) : Resource<T>()
    ...
    fun <R> map(transform: (T?) -> R?): Resource<R> {
        return when (this) {
            ...
            is Progress -> loading(this.progression, transform(this.data))
    ...
    companion object {
        ...
        fun <T> loading(progression: Int? = null, data: T? = null): Resource<T> =
